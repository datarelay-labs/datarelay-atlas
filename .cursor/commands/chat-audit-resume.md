...
4. Load the durable Audit Control Packet for this repository from GitHub
   Contents API blob-SHA CAS. Discover Issue N automatically when unset
   (ACTIVE_CHECKPOINT_ISSUE pointer or ACTIVE WORKSTREAM Issue); do not invent N:
   - `python -m atlas chat-audit show --repository <owner/repo> --worktree "$PWD"`
...
6. Execute exactly one bounded audit slice:
   `python -m atlas chat-audit run-slice --repository <owner/repo> --worktree "$PWD"`
